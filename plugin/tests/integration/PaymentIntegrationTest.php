<?php
/**
 * Pruebas de integración entre pedidos y el módulo de pagos.
 *
...
final class PaymentIntegrationTest extends WP_UnitTestCase {
	/**
	 * Instala y aísla dominio y pagos.
	 *
	 * @return void
	 */
	...
	public function test_checkout_creates_one_idempotent_payment_request(): void {
		...
		$this->assertGreaterThan( 0, $record['payment_request_id'] );
		...
		$this->assertMatchesRegularExpression( '/T\d{2}:\d{2}:\d{2}\+00:00$/', PaymentRequests::get( $record['payment_request_id'] )['expires_at'] );
		...
		$this->assertSame( $record['payment_request_id'], OrderService::payment_record( $order['public_id'] )['payment_request_id'] );
		$this->assertSame( 1, $this->count_payment_requests() );
	}
	...
	public function test_reconciliation_recovers_lost_hook(): void {
		...
		$record    = OrderService::payment_record( $scenario['order']['public_id'] );
		$confirmed = $this->without_event_listeners(
			'vicu_pagos_confirmado',
			static fn (): array|WP_Error => PaymentRequests::transition( $record['payment_request_id'], PaymentRequestState::CONFIRMED, 2 )
		);
		$this->assertNotWPError( $confirmed );
	...
	public function test_payment_mismatch_creates_safe_alert(): void {
		...
		$record   = OrderService::payment_record( $scenario['order']['public_id'] );
		$this->tamper_amount( $record['payment_request_id'] );
	...
	/**
	 * Cuenta las solicitudes de pago almacenadas.
	 *
	 * @return int
	 */
	private function count_payment_requests(): int {
		global $wpdb;

		$table = \Vicu\Restaurante\Payments\PaymentRequestRepository::table_name();

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Tabla propia del plugin en la base aislada de pruebas.
		return (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$table}" );
	}

	/**
	 * Ejecuta una acción con los listeners de un hook retirados y los restaura.
	 *
	 * @param string   $hook     Hook cuyos listeners se retiran.
	 * @param callable $callback Acción que se ejecuta sin listeners.
	 * @return mixed
	 */
	private function without_event_listeners( string $hook, callable $callback ): mixed {
		global $wp_filter;
		...
		try {
			...
			if ( null !== $saved ) {
				$wp_filter[ $hook ] = $saved; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Restaura el estado global alterado por la prueba.
			}
		}
	}

	/**
	 * Altera el monto persistido para simular una divergencia.
	 *
	 * @param int $request_id ID del post de la solicitud de pago.
	 * @return void
	 */
	private function tamper_amount( int $request_id ): void {
		global $wpdb;

		$table = \Vicu\Restaurante\Payments\PaymentRequestRepository::table_name();

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching,WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Tabla propia del plugin en la base aislada de pruebas.
		$wpdb->query( $wpdb->prepare( "UPDATE {$table} SET amount_minor = amount_minor + 1, revision = revision + 1 WHERE post_id = %d", $request_id ) );
	}
